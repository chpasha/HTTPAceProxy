...
class HTTPHandler(BaseHTTPServer.BaseHTTPRequestHandler):
    ...
    def handleRequest(self, headers_only, channelName=None, channelIcon=None, fmt=None):
        ...
        try:
            ...
            self.dieWithError(400, 'Bad Request', logging.WARNING); return  # 400 Bad Request
        ...
        try:
            ...
            if AceConfig.acerestartondatatimeout:
                logger.warning('Timeout receiving data, scheduling restart')
        ...
        try:
            while self.handler.connection and self.ace._streamReaderState == 2:
                try:
                    ...
                    raise ReadDataTimeoutError('No data received within timeout')
        finally:
            if transcoder is not None:
                try:
                    transcoder.kill(); logger.warning('Ffmpeg transcoding stoped')
                except:
                    pass
...
def spawnAce(cmd, delay=0.1):
    if AceConfig.osplatform == 'Windows':
        ...
        try:
            ...
            logger.error("Can't find acestream!"); sys.exit(1)
        else:
...
def checkFirewall(clientip):
    try:
        clientinrange = any([IPAddress(clientip) in IPNetwork(i) for i in AceConfig.firewallnetranges])
    except:
        logger.error('Check firewall netranges settings !'); return False
